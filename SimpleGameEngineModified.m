% Simple Game Engine (Modified)
% Modified by Eli Lambrych
%
...
classdef SimpleGameEngineModified < handle
    ...
    methods
        function obj = SimpleGameEngineModified(sprites_fname, sprite_height, sprite_width, zoom, background_color, disableMargins)
            ...
            if (disableMargins)
                sprite_row_max = floor((sprites_size(1)+1)/(sprite_height));
                sprite_col_max = floor((sprites_size(2)+1)/(sprite_width));
            else
                sprite_row_max = floor((sprites_size(1)+1)/(sprite_height+1));
                sprite_col_max = floor((sprites_size(2)+1)/(sprite_width+1));
            end

            obj.sprite_row_max = sprite_row_max;
            obj.sprite_col_max = sprite_col_max;
